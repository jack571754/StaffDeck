...
### 4. 推送模块 (Notify)
- **多任务与多员工分流路由与关注人规范**：
  - **任务级路由 (`task_routes`)**：支持同一员工名下不同任务推送不同群；在 `config.json` 的 `task_routes` 中按任务 ID 或标题关键字独立配置；
  - **任务Prompt直接传参**：支持执行脚本时传入 `--task-title <标题>`、`--webhook-url <URL>`、`--notify-users <人员列表>`、`--mention-all`，天然保证任务间完全独立；
  - **员工级兜底 (`agent_routes`)**：任务未单独配置时，回退到数字员工维度的专属群与提醒人；
  - **飞书卡片强提醒**：飞书卡片 2.0 自动装配 `<at id="open_id">姓名</at>` 或 `<at id="all">所有人</at>` 标签；
- **卡片装配**：原生组装飞书交互式卡片 2.0（Header 色带 + 顶部指标概览栏 + 正负增量Top5双栏 + 自适应表格 + 关注人@提醒 + 底部防重注释）；
