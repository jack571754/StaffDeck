...
- **默认支持卡片与表格（`msg_type=interactive`）**：在 Markdown 正文中直接使用 `| 列1 | 列2 |` 表格语法，脚本会自动将其转换为飞书 Card Schema 2.0 原生表格组件（`tag: "table"`），支持首列冻结、金额/数值列智能右对齐、高紧凑行高与内置分页！
- 自动提取正文首行作为卡片带色彩的标题栏（`header`），让消息更突出醒目。
